...
def parse_passport_text(text, filename):
    # Ищем дату в тексте (форматы ДД.ММ.ГГГГ или ГГГГ.ММ.ДД) или в названии файла
    ...
    # Материал и поставщик (берем имя файла как основу для наименования, если внутри не вычленилось)
    clean_name = re.sub(r'\.(pdf|jpg|png|jpeg)$', '', filename, flags=re.IGNORECASE)
    clean_name = re.sub(r'^\d{4}\.\d{2}\.\d{2}\s*', '', clean_name) # убираем дату из начала
    material_name = clean_name

    # Ищем поставщика/изготовителя по ключевым словам
    supplier = ""
    lines = text.split('\n')
    for line in lines:
...
def main():
    ...
    print("\nГотово! Реестр собран локальным OCR.")
